 ...
 */
odoo.define('Pos_Discount_Limit.models_screen', function (require) {
    "use strict";
    ...
    console.log("aaaaaaaaaaaaaaaaaaaaahhhhhhhhhhhhhhhhhhhhhhhhhhh");
    Orderline.Orderline.prototype.constructor = function(obj, options) {
        console.log("iam here :Orderline.Orderline.prototype.constructor = function(obj, options)");
    ...
    Orderline.Orderline.prototype.getUnitDisplayPriceBeforeDiscount= function(){
    ...
    Orderline.Orderline.prototype.set_discount = function(discount) {
        console.log("iam here :set_discount");
    ...
    Orderline.Orderline.prototype.get_base_price = function() {
        console.log("iam here :get_base_price",this.discount_type);
    ...
    //         if (this.discount_type === 'fixed') {
    //             console.log(totalDiscount,'hiiiiiiiiiiiiiiiiiiiiiiiiiiiii from total discount ');

    //             sum += orderLine.get_discount() * orderLine.get_quantity();
    ...
    // },
    console.log(Orderline.Order.prototype.get_total_discount,'ffsdfsdfsdfsd');
    Orderline.Order.prototype.get_total_discount = function() {
        ...
        /*let totalDiscount = 0;
        const ignoredProductIds = this._get_ignored_product_ids_total_discount(); // Define this method to get ignored product IDs
        (this.order.orderlines || []).forEach(orderLine => {
            if (!ignoredProductIds.includes(orderLine.product.id)) {
                if (orderLine.discount_type === 'fixed') {
                    totalDiscount += orderLine.get_discount() * orderLine.get_quantity();
                } else if (orderLine.discount_type === 'percentage') {
                    totalDiscount += (orderLine.get_unit_display_price_before_discount() * (orderLine.get_discount() / 100)) * orderLine.get_quantity();
                }
            }
        });
    
        // If the discount type is 'fixed', round the total discount to match the currency rounding
        if (this.discount_type === 'fixed') {
            const rounding = this.pos.currency.rounding;
            totalDiscount = round_pr(totalDiscount, rounding);
        }
    ...
    Orderline.Orderline.prototype.get_discount_str=function(){
        console.log(this.discountStr,"hhhhh")
        return this.discountStr;
    }
    Orderline.Orderline.prototype.set_selected= function(selected){
    ...
    Orderline.Orderline.prototype.get_subtotal= function(){
        return round_pr(this.orderlines.reduce((function(sum, orderLine){
            if(orderLine.discount_type=='fixed'){
    ...
    Orderline.Orderline.prototype.getOrderReceiptEnv=function() {
    ...
    Orderline.Orderline.prototype.export_for_printing = function() {
        return {
            ...
            total_discount: this.get_discount(),
